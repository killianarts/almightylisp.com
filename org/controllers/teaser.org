#+title: Teaser Controller
#+property: header-args:common-lisp :tangle ../../src/controllers/teaser.lisp

The landing page controller. Renders a series of animated slides that
introduce Lisp concepts (lists, symbols, functions, macros, etc.) with
code examples.
...
Returns a closure that appends an incrementing counter to each string it
receives -- used to generate unique slide IDs.
...
The main teaser page: a hero title followed by slides walking through
Lisp fundamentals.

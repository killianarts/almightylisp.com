...
(defmethod node-dispatchers ((node org-node))
  (or *dispatchers* 
      (mapcar #'make-instance '(src-node properties-node outline-node text-node))))
...
(defmethod node-start ((node text-node) stack)
  (let ((newline-pos (position #\Newline stack)))
    (when newline-pos
      (let ((line-chars (subseq stack 0 (1+ newline-pos)))
            (old-stack (nthcdr (1+ newline-pos) stack)))
        (values (make-instance 'text-node
                               :text (stack->string line-chars))
                old-stack)))))(in-package :cl-org-mode)
